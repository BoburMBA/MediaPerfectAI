...
export default function MusicGenPage() {
  ...
  const [vocalStyle, setVocalStyle] = useState<'male' | 'female' | 'duet'>('duet');
    ...
    try {
      ...
      return Array.isArray(parsed) 
        ? parsed.filter(a => a.type === 'music').map(a => ({
            id: a.id, name: a.name, prompt: a.metadata?.prompt || '', url: a.url, createdAt: a.createdAt
          }))
        : [];
    } catch {
      return [];
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-center text-center md:text-left">
                         ...
                         <div className="mt-6 flex items-center justify-center md:justify-start gap-4">
                           ...
                           <div className="ml-auto flex items-center gap-2 px-3 py-1.5 bg-green-500/10 border border-green-500/20 rounded-full">
                              ...
                              <span className="text-[9px] font-bold text-green-400 uppercase tracking-widest">Auto-Saved</span>
                           </div>
                         </div>
                      </div>
                   </div>
