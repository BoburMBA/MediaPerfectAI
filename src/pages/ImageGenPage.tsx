  ...
  const generateImage = async () => {
    ...
    try {
      ...
      if (config.provider === 'openai' || config.provider === 'minimax') {
        ...
        // Fallback to pollinations.ai for free AI image generation
      ...
      // We pre-load if it's OpenAI or Minimax to ensure image exists before rendering
      if (config.provider === 'openai' || config.provider === 'minimax') {
        const img = new Image();
        img.src = imageUrl;
        await new Promise((resolve, reject) => {
          ...
          img.onerror = () => {
            clearTimeout(timeout);
            reject(new Error('Failed to load image from generated URL. The link might be restricted or expired.'));
          };
        });
      }
      ...
      const newAsset = {
        id: newImage.id,
        name: newImage.prompt.substring(0, 20) + (newImage.prompt.length > 20 ? '...' : ''),
        type: 'image',
        url: newImage.url,
        createdAt: newImage.createdAt,
        metadata: { prompt: newImage.prompt }
      };
            ...
            <div className="flex-1 flex items-center justify-center min-h-0 relative">
              <AnimatePresence mode="wait">
                {isGenerating ? (
                  ...
                  >
                    <div className="aspect-square bg-black rounded-3xl overflow-hidden shadow-2xl shadow-black/60 border border-white/5 p-4 bg-gradient-to-br from-white/5 to-transparent">
                      <img src={currentImage.url} alt="Generated" className="w-full h-full object-cover rounded-2xl" />
                    </div>
                    
                    <div className="absolute top-8 right-8 flex gap-2">
                       <div className="px-4 py-2 bg-black/60 backdrop-blur-xl rounded-2xl border border-white/10 flex items-center justify-center gap-2 shadow-xl">
                          <span className="text-green-400 text-xs font-bold uppercase tracking-widest flex items-center gap-1">
                             <div className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse" /> Auto-Saved
                          </span>
                       </div>
                       <button 
                        onClick={() => {
                          ...
                          a.download = `vision-${currentImage.id}.png`;
                          a.click();
                        }}
                ...
                history.map((img) => (
                  <div 
                    ...
                    className={`group relative aspect-square rounded-2xl overflow-hidden border-2 transition-all cursor-pointer shadow-lg ${
                      currentImage?.id === img.id ? 'border-orange-500 scale-[1.02]' : 'border-white/5 hover:border-white/20'
                    }`}
                  >
